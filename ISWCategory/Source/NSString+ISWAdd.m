...
{
    return [self attributedStringWithStyleBook:[self getFontStyleDic]];
}

-(NSDictionary *)getFontStyleDic
{
...
{
    NSString        *priceStr = [NSString isw_shortPrice:price];

    NSDictionary    *style1         = @{
                                        @"unit": @[
                                                [UIFont isw_Pingfang:18.0]
                                                ],
                                        @"price": @[
                                                [UIFont isw_Pingfang:24.0]
                                                ]
                                        };

    //字串组合
    NSString *priceFormat = [NSString stringWithFormat:@"<unit>￥</unit><price>%@</price>", priceStr];
    
    return [[NSMutableAttributedString alloc] initWithAttributedString:[priceFormat attributedStringWithStyleBook:style1]];
}
...
+ (NSMutableAttributedString*)isw_summarylPrice:(CGFloat)price
{
    ...
    //字串组合
    NSString *priceFormat = [NSString stringWithFormat:@"<unit>￥</unit><price>%0.2f</price>", price];
...
+ (NSMutableAttributedString*)isw_balanceAmount:(CGFloat)amount
{
    ...
    NSDictionary    *style1         = @{
                                        @"unit": @[
                                                [UIFont isw_Pingfang:20.0]
                                                ],
                                        @"yuan": @[
                                                [UIFont isw_Pingfang:30.0]
                                                ],
                                        @"cent": @[
                                                [UIFont isw_Pingfang:20.0]
                                                ],
    ...
    //字串组合
    NSInteger yuan = (NSInteger)amount;
    CGFloat   cent = (amount - (float)yuan)*100.00;

    NSString *priceFormat = [NSString stringWithFormat:@"<unit>￥</unit><yuan>%ld.</yuan><cent>%02.0f</cent>", yuan, cent];
...
+ (NSMutableAttributedString*)isw_detailPrice:(CGFloat)price
{
    ...
    //字串组合
    NSInteger yuan = (NSInteger)price;
    CGFloat   cent = (price - (float)yuan)*100.00;
...
+ (NSMutableAttributedString*)showRemainingTime:(int)remainingSeconds
{
    ...
    if (remainingSeconds<=0) {
        return [[NSMutableAttributedString alloc] initWithAttributedString:[strCountDown attributedStringWithStyleBook:style1]];
    }
    
    int hours   = remainingSeconds / OneHour;
    int mintues = (remainingSeconds / OneMintue) % OneMintue;
    int seconds = remainingSeconds % OneMintue;
    ...
    //字串组合
    if (hours > 0) {
        strHours = [NSString stringWithFormat:@"<time>%d</time><body>小时</body>", hours];
    }
    
    if (hours > 0 || mintues > 0) {
        strMintues = [NSString stringWithFormat:@"<time>%d</time><body>分</body>", mintues];
    }
    
    strSeconds = [NSString stringWithFormat:@"<time>%02d</time><body>秒</body>", seconds];
    
    strCountDown = [NSString stringWithFormat:@"<body>剩余</body>%@%@%@",strHours,strMintues,strSeconds];
