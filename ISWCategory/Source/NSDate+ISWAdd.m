...
-(BOOL)isToday:(NSDate*) date{
    ...
    //差值为0天
    return [self calWithValue:0 withTargetDate:date];
}
...
-(BOOL)isYesToday:(NSDate*) date{
    ...
    //差值为1天
    return [self calWithValue:1 withTargetDate:date];
}

-(BOOL)calWithValue:(NSInteger)value withTargetDate:(NSDate*) targetDate{
    
    //得到给定时间的处理后的时间的components
    NSDateComponents *dateComponents=self.ymdDate.components;
    
    //得到当前时间的处理后的时间的components
    NSDateComponents *targetComponents=targetDate.ymdDate.components;

    //比较
    BOOL res=dateComponents.year==targetComponents.year && dateComponents.month==targetComponents.month && (dateComponents.day + value)==targetComponents.day;
